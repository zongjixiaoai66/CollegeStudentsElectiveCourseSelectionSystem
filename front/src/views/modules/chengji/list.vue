        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '成绩标题' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.chengjiName" placeholder="成绩标题" clearable></el-input>
                     </el-form-item>
         
                     <el-form-item :label="contents.inputTitle == 1 ? '成绩类型' : ''">
                        <el-select v-model="searchForm.chengjiTypes" placeholder="请选择成绩类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in chengjiTypesSelectSearch"
                               v-bind:key="index"
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '学生名称' : ''">
                        <el-input prefix-icon="el-icon-search" v-model="searchForm.yonghuName" placeholder="学生名称" clearable></el-input>
                    </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('chengji','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/daxueshengxuanxiuxuanke/upload/chengjiMuBan.xls"
                        >批量导入学生成绩数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('chengji','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="chengjiUploadSuccess"
                                :on-error="chengjiUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('chengji','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入学生成绩数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('chengji','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "chengji.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
                ...
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('chengji','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('chengji','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('chengji','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'chengji',//当前表表名,
                        sumColum : 'chengji_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'chengji_name',//分组字符串字段
                        // types : 'chengji_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.chengjiName!= '' && this.searchForm.chengjiName!= undefined) {
                    params['chengjiName'] = '%' + this.searchForm.chengjiName + '%'
                }
         
                if (this.searchForm.chengjiTypes!= '' && this.searchForm.chengjiTypes!= undefined) {
                    params['chengjiTypes'] = this.searchForm.chengjiTypes
                }
                                        
                params['chengjiDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=chengji_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.chengjiTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            chengjiUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "chengji/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
